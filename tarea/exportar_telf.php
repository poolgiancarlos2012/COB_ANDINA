...
require_once 'C:/xampp/htdocs/COB_ANDINA/factory/FactoryConnection.php';
 ...
 function limpiarCartera4($name) {
    ...
    $path = "C:/xampp/htdocs/COB_ANDINA/documents/loadtelf/".$name;
    if (!file_exists($path)) {
        echo "no existe o fue removida ".$name." ".date("Y-m-d H:m:s")."\n";
        exit();
    }    
    $tmp_file = "TMP" . session_id() . rand(1, 1000) . $name;
    $tmpArchivo = @fopen("C:/xampp/htdocs/COB_ANDINA/documents/loadtelf/".$tmp_file, 'a+');
    if (!$tmpArchivo) {
    ...
    if ($carteraArchivo) {
        ...
        fclose($tmpArchivo);
        $insertfileload=" INSERT INTO loadtelf(nombrecarga,nombretrans,pendiente) VALUES('$name','$tmp_file',1);";
		$prinsertfileload = $connection->prepare($insertfileload);
		$prinsertfileload->execute();        
        echo "Exportar Telf ".$name."\n";
    } else {
        fclose($carteraArchivo);
        fclose($tmpArchivo);
        echo "Error"."\n";
