...
if(!empty($dataasunto) AND count($dataasunto)==1){
		...
		$sqlcor="	SELECT cor.idcorreo,cor.correo FROM ca_correo_envio cor WHERE fecha_envio IS NULL LIMIT 20 ";
		...
		while($datos_cor=$prcor->fetch(PDO::FETCH_ASSOC)) {
			...
			$layout = file_get_contents('C:/xampp/htdocs/COB_ANDINA/tarea/layout/layout_envio_importante.php');
			...
			$asunto="Grupo Andina - Informacion Importante";
			...
			$objMailer->AddCC('jsosa@grupoandina.com.pe');
			$objMailer->AddCC('mgonzales@grupoandina.com.pe');
