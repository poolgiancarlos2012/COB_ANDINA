...
/*$sql = <<<EOT
	...
	WHERE dir.idtipo_referencia=3 AND cuen.dato1='{$fecha_proceso}' AND cuen.idcartera IN ({$carteras}) and fin_ser.idservicio={$servicio} and cuen.estado=1
	...
	SELECT DISTINCT 
		...
		LEFT( CONCAT_WS('', IF(trim(dir.direccion)!='' AND dir.departamento is not null AND dir.provincia is not null and dir.distrito is not null and dir.departamento!='' and dir.provincia!='' and dir.distrito!='',dir.direccion,''), '                                                            ') , 50) AS direccion,
		LEFT( CONCAT_WS('', IF(dir.departamento REGEXP ('^[0-9]')=0,'',dir.departamento), '    '), 2) AS departamento,
		LEFT( CONCAT_WS('', IF(dir.provincia REGEXP ('^[0-9]')=0,'',dir.provincia), '    '), 2) AS provincia,
		LEFT( CONCAT_WS('', IF(dir.distrito REGEXP ('^[0-9]')=0,'',dir.distrito), '    '), 3) AS distrito,
		LEFT( CONCAT_WS('',DATE_FORMAT(MEJOR.fecha_cp, '%d/%m/%Y'),'            '), 10) AS fecha_compromiso,
...
$sql = <<<EOT
	...
	FROM ca_cliente_cartera cli_car 
		...
		INNER JOIN ca_final_servicio fin_ser ON MEJOR.idfinal=fin_ser.idfinal
	WHERE cuen.idcartera IN ({$carteras}) and fin_ser.idservicio={$servicio} 
	GROUP BY MEJOR.idcliente_cartera,cuen.idcuenta,MEJOR.fecha_visita
...
//exit();
	...
	$buscar= array("Ã‘","Ã","Ã±","±","á","é", "í","ó","ú", "É","Ó","Ú", "Ã","Ã¡", "Ã", "Ã?", "Ã©", "Ãš", "Ã",  "Ã",  "Ã­", "Ã¬", "Ã?",  "Ã", "Ã³", "Ã²",  "Ã",  "Ã", "Ãº", "Ã¹", "Ã",   "Ã","Ã?", "Â", "Ã","â",  "*", "Ã" , "À", "Á", "Â", "Ã", "Ä", "Å", "Æ", "Ç", "È", "É", "Ê", "Ë", "Ì","Í","Î","Ï","Ð","Ñ","Ò","Ó","Ô","Õ","Ö","×","Ø","Ù","Ú","Û","Ü","Ý","Þ","ß","à","á","â","ã","ä","å","æ","ç","è","é","ê","ë","ì","í","î","ï","ð","ñ","ò","ó","õ","ö","÷","ø","ù","ú","û","ü","ý","þ","ÿ","Œ","œ","Š","š","Ÿ","ƒ","†","‡","‰","€");
