...
header('content-disposition:atachment;filename=visita_por_cliente.xls');
...
$sql="SELECT cli.nombre,CONCAT('=\"',cli.codigo,'\"') AS codigo,CONCAT('=\"',cu.numero_cuenta,'\"') as numero_cuenta,GROUP_CONCAT(IFNULL(DATE(vis.fecha_visita),''),'@@',carfin.nombre,'@@',fin.nombre,'@@',vis.observacion,'@@',IFNULL(dir.distrito,''),'@@',IFNULL(dir.provincia,''),'@@',IFNULL(dir.departamento,'') SEPARATOR '||') as VISITAS FROM ca_cliente cli
...
inner join ca_visita vis
on vis.idcuenta=cu.idcuenta
inner join ca_final fin
on fin.idfinal=vis.idfinal
inner join ca_carga_final carfin
on carfin.idcarga_final=fin.idcarga_final
inner join ca_direccion dir
on dir.iddireccion=vis.iddireccion
