<?php
	...
	header("Content-Disposition: attachment; filename=fotocartera_cliente_con_gestion.xls");
	header("Expires: 0");
	...
	$sqlLV = " SELECT CONCAT('=\"',clicar.codigo_cliente,'\"') AS 'CODIGO_CLIENTE', 
			...
			( SELECT departamento FROM ca_direccion WHERE idcliente_cartera = clicar.idcliente_cartera AND idcartera = clicar.idcartera LIMIT 1 ) AS 'DEPARTAMENTO',
                        ...
                        ".implode(",",$field)." ".$str_implode_pago.", 
			...
			( SELECT nombre FROM ca_final WHERE idfinal = cu.ml_estado LIMIT 1 ) AS 'ML_ESTADO', ( SELECT niv.nombre FROM ca_final fin INNER JOIN ca_nivel niv ON niv.idnivel = fin.idnivel  WHERE fin.idfinal = cu.ml_estado LIMIT 1 ) AS 'ML_PERFIL_CLIENTE',
			...
			".$join_pago." 
			WHERE clicar.id_ultima_llamada != 0 AND cli.idservicio = ? AND clicar.idcartera = ? AND cu.idcartera = ? AND det.idcartera = ? ".$where_pago." ";
