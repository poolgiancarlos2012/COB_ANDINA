<?php
    ...
    $sql_llamada="SELECT $cabeceras,ML_CUENTA,NUEVO_FONO,'',CORREO
                ...
                IFNULL((select CONCAT(IFNULL(DATE(lla.fecha),' '),'@@',IFNULL(fin.nombre,' '),'@@',IFNULL(tel.numero,' '),'@@',IFNULL(carfin.nombre,' '),'@@',IFNULL(lla.nombre_contacto,''),'@@',IFNULL(fin.nombre,''),'@@',IFNULL(replace(REPLACE(Replace(Replace(Replace(lla.observacion,'|',' '),char(10),' '),char(13),' '),CHAR(9),' '),char(8),' '),' '),'@@',IFNULL(lla.fecha_cp,' '),'@@',IFNULL(lla.monto_cp,' '))
                from ca_llamada lla
                ...
                INNER JOIN ca_clase_final clafin ON clafin.idclase_final=fin.idclase_final
                ...
                where lla.idcuenta=cu.idcuenta and DATE(lla.fecha) BETWEEN '$fecha_inicio' AND '$fecha_fin'
                ORDER BY finser.peso DESC,lla.fecha DESC limit 1),'@@@@@@@@@@@@@@@@') AS 'ML_CUENTA'                 
                 ...
                 WHERE clicar.idcartera in (".$cartera.") and clicar.estado=1)A";
