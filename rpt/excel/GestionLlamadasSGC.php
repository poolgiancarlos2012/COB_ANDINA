<?php
    ...
    */
    $sql = "SELECT 
        ...
        FROM ca_cartera car INNER JOIN ca_cliente cli INNER JOIN ca_cliente_cartera clicar INNER JOIN ca_llamada lla INNER JOIN ca_cuenta cu INNER JOIN ca_final fin 
        ON fin.idfinal = lla.idfinal AND cu.idcuenta = lla.idcuenta AND lla.idcliente_cartera = clicar.idcliente_cartera AND clicar.idcliente = cli.idcliente AND clicar.idcartera=car.idcartera
        WHERE clicar.idcartera IN (" . $cartera . ") AND cu.idcartera IN ( ".$cartera." ) 
        AND cli.idservicio = $servicio 
        AND DATE(lla.fecha_creacion) BETWEEN '" . $fecha_inicio . "' AND '" . $fecha_fin . "' 
        AND car.idcartera IN (" . $cartera . ")	group by car.nombre_cartera,clicar.codigo_cliente,lla.fecha" ;
