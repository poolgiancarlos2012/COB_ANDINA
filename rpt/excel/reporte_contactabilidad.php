...
$style_borde_negro = array(
 ...
 $style_solido_rojo=                   array(
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A2','REPORTE DE CONTACTABILIDAD DEL '.$fecha_unica);
	...
	$sql_total_mejor_llamada="SELECT IFNULL(SUM(IF(nombre='CEF' or nombre='CNE',1,0)),0) as 'CONTACTO',IFNULL(SUM(IF(nombre='NOC',1,0)),0) as 'NO_CONTACTO',COUNT(*) AS 'TOTAL',
							...
							where finser.idservicio=$servicio and clicar.idcartera = $cartera and lla.tipo NOT IN ('IVR') and date(lla.fecha)='".$fecha_unica."' and cu.dato9 in ($territorio) AND (lla.idusuario_servicio!=1370 OR carfin.nombre!='CNE')
...
$sql_contacto_titular="SELECT TOTAL_LLAMADA.estado,IFNULL(MEJOR_LLAMADA.cantidad,0) as cantidad1,TOTAL_LLAMADA.cantidad as cantidad2 FROM (
				...
				from ca_cliente_cartera clicar inner join ca_llamada lla inner join ca_final fin inner join ca_final_servicio finser inner join ca_carga_final carfin inner join ca_cuenta cu
				on finser.idfinal = fin.idfinal and fin.idfinal = lla.idfinal and lla.idcliente_cartera= clicar.idcliente_cartera and fin.idcarga_final=carfin.idcarga_final and cu.idcliente_cartera=clicar.idcliente_cartera and cu.idcuenta=lla.idcuenta
				where finser.idservicio=$servicio and clicar.idcartera = $cartera and lla.tipo NOT IN ('IVR') and date(lla.fecha)='$fecha_unica' and carfin.nombre in ('CEF') and cu.dato9 in ($territorio) AND (lla.idusuario_servicio!=1370 OR carfin.nombre!='CNE') group by lla.idcliente_cartera,lla.fecha,lla.fecha_cp,lla.monto_cp,lla.idtelefono,lla.observacion
...
$sql_contacto_tercero="SELECT TOTAL_LLAMADA.estado,IFNULL(MEJOR_LLAMADA.cantidad,0) as cantidad1,TOTAL_LLAMADA.cantidad as cantidad2 FROM (
				...
				from ca_cliente_cartera clicar inner join ca_llamada lla inner join ca_final fin inner join ca_final_servicio finser inner join ca_carga_final carfin inner join ca_cuenta cu
				on finser.idfinal = fin.idfinal and fin.idfinal = lla.idfinal and lla.idcliente_cartera= clicar.idcliente_cartera and fin.idcarga_final=carfin.idcarga_final and cu.idcliente_cartera=clicar.idcliente_cartera and cu.idcuenta=lla.idcuenta
				where finser.idservicio=$servicio and clicar.idcartera = $cartera and lla.tipo NOT IN ('IVR') and date(lla.fecha)='$fecha_unica' and carfin.nombre in ('CNE') and cu.dato9 in ($territorio) AND (lla.idusuario_servicio!=1370 OR carfin.nombre!='CNE')  group by lla.idcliente_cartera,lla.fecha,lla.fecha_cp,lla.monto_cp,lla.idtelefono,lla.observacion
				)A GROUP BY A.idfinal,A.estado order by cantidad DESC
)TOTAL_LLAMADA 
LEFT JOIN(
				SELECT A.idfinal,A.estado,count(*) as cantidad FROM (
...
$sql_no_contacto="SELECT TOTAL_LLAMADA.estado,IFNULL(MEJOR_LLAMADA.cantidad,0) as cantidad1,TOTAL_LLAMADA.cantidad as cantidad2 FROM (
				...
				from ca_cliente_cartera clicar inner join ca_llamada lla inner join ca_final fin inner join ca_final_servicio finser inner join ca_carga_final carfin inner join ca_cuenta cu
				on finser.idfinal = fin.idfinal and fin.idfinal = lla.idfinal and lla.idcliente_cartera= clicar.idcliente_cartera and fin.idcarga_final=carfin.idcarga_final and cu.idcliente_cartera=clicar.idcliente_cartera and cu.idcuenta=lla.idcuenta
				where finser.idservicio=$servicio and clicar.idcartera = $cartera and lla.tipo NOT IN ('IVR') and date(lla.fecha)='$fecha_unica' and carfin.nombre in ('NOC') and cu.dato9 in ($territorio)  AND (lla.idusuario_servicio!=1370 OR carfin.nombre!='CNE') group by lla.idcliente_cartera,lla.fecha,lla.fecha_cp,lla.monto_cp,lla.idtelefono,lla.observacion
				)A GROUP BY A.idfinal,A.estado order by cantidad DESC
)TOTAL_LLAMADA 
LEFT JOIN(
				SELECT A.idfinal,A.estado,count(*) as cantidad FROM (
...
/*    $sql_fotocartera="select fproceso,agencia,territorio,producto,nom_subprod,CONCAT('=\"',contrato,'\"') as contrato,CONCAT('=\"',codcent,'\"') AS codcent,nombre,divisa,
				...
				IFNULL((select observacion from ca_llamada where idcliente_cartera=clicar.idcliente_cartera and DATE(fecha)<='".$fecha_unica."' order by fecha DESC limit 1),'') as 'Observacion',
				IFNULL((select CONCAT(usu.paterno,' ',usu.materno,' ',usu.materno) from ca_llamada lla inner join ca_usuario_servicio ususer on lla.idusuario_servicio=ususer.idusuario_servicio inner join ca_usuario usu on usu.idusuario=ususer.idusuario where lla.idcliente_cartera=clicar.idcliente_cartera and DATE(lla.fecha)<='".$fecha_unica."' order by fecha DESC limit 1),'') as 'Operador'
				from ca_historial his
				inner join ca_cliente_cartera clicar on his.idcliente_cartera=clicar.idcliente_cartera
				where clicar.idcartera=$cartera and his.fproceso in ($fproceso)
...
*/
	...
	$sql_fotocartera="select fproceso,agencia,territorio,producto,nom_subprod,CONCAT('=\"',contrato,'\"') as contrato,CONCAT('=\"',codcent,'\"') AS codcent,replace(nombre,\"'\",'') as nombre,divisa,
				saldohoy,diavenc,ubigeo,dpto,dist_prov,tramo_dia,marca,
				...
				replace(replace(REPLACE(Replace(Replace(Replace(IFNULL(A.observacion,''),'|',''),char(10),''),char(13),''),CHAR(9),''),char(8),''),char(61),'') as 'Observacion',
				...
				LEFT OUTER JOIN (
									...
									(
										...
										on finser.idfinal = fin.idfinal and fin.idfinal = lla.idfinal and lla.idcliente_cartera= clicar.idcliente_cartera and carfin.idcarga_final=fin.idcarga_final
										where finser.idservicio=$servicio and clicar.idcartera = $cartera and finser.idservicio = $servicio and DATE(lla.fecha)<='$fecha_unica' AND (lla.idusuario_servicio!=1370 OR carfin.nombre!='CNE')
											and lla.tipo NOT IN ('IVR') 
										order by lla.idcliente_cartera, finser.peso desc,lla.fecha DESC 
								) t1 group by t1.idcliente_cartera
    ...
    $objPHPExcel->getActiveSheet()->getStyle('A1:X1')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
   	$objPHPExcel->getActiveSheet()->getStyle('A1:X1')->getFill()->getStartColor()->setARGB('FF963634');
	$objPHPExcel->getActiveSheet()->getStyle('A1:X1')->getFont()->setSize(12);
	$objPHPExcel->getActiveSheet()->getStyle('A1:X1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	$objPHPExcel->getActiveSheet()->getStyle('A1:X1')->getFont()->setColor(new PHPExcel_Style_Color(PHPExcel_Style_Color::COLOR_WHITE));
...
//	exit();
	...
	$sql_detalle="select cu.dato9 as 'Territorio',CONCAT('=\"',clicar.codigo_cliente,'\"') as 'CodCent',Date(fecha) as 'fecha',time(fecha) as 'hora',
					...
					carfin.nombre as 'Tipo',fin.nombre as 'Estado',lla.fecha_cp as 'F.Cpg',replace(replace(REPLACE(Replace(Replace(Replace(lla.observacion,'|',''),char(10),''),char(13),''),CHAR(9),''),char(8),''),'=','') as 'Obs',
					...
					(select Prioridad from ca_final_servicio finser where finser.idfinal=lla.idfinal and finser.idservicio=$servicio) as 'Prioridad'
					from ca_llamada lla
					...
					where clicar.idcartera=$cartera and lla.tipo NOT IN ('IVR') and date(fecha)='".$fecha_unica."' and cu.dato9 in ($territorio) AND (lla.idusuario_servicio!=1370 OR carfin.nombre!='CNE') group by lla.idcliente_cartera,lla.fecha,lla.fecha_cp,lla.monto_cp,lla.idtelefono,lla.observacion";

    $objPHPExcel->getActiveSheet()->getStyle('A1:K1')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
   	$objPHPExcel->getActiveSheet()->getStyle('A1:K1')->getFill()->getStartColor()->setARGB('FF963634');
	$objPHPExcel->getActiveSheet()->getStyle('A1:K1')->getFont()->setSize(12);
	$objPHPExcel->getActiveSheet()->getStyle('A1:K1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	$objPHPExcel->getActiveSheet()->getStyle('A1:K1')->getFont()->setColor(new PHPExcel_Style_Color(PHPExcel_Style_Color::COLOR_WHITE));
