<?php
	...
	if( $dataTabla == '' || $dataTabla == 'temporal eliminada' ) {
		...
		$sqlLV = " SELECT tmp.*, ".$str_implode_pago." 
				TRUNCATE(cu.monto_pagado,2) AS MONTO_PAGADO_REAL,
				...
				cu.idcliente_cartera AS CODABONADO_NEXTEL,
				...
				(SELECT CONCAT_WS(' ',usu.paterno,usu.materno,usu.nombre) FROM ca_usuario_servicio ususer INNER JOIN ca_usuario usu ON usu.idusuario = ususer.idusuario WHERE ususer.idusuario_servicio=clicar.idusuario_servicio ) AS 'OPERADOR_ASIGNADO',
				(SELECT CONCAT_WS(' ',usu.paterno,usu.materno,usu.nombre) FROM ca_usuario_servicio ususer INNER JOIN ca_usuario usu ON usu.idusuario = ususer.idusuario WHERE ususer.idusuario_servicio=clicar.idusuario_servicio_especial ) AS 'OPERADOR_SEGUIMIENTO'				
				FROM ca_cuenta cu 
