<?php
	...
	//~ Vic I
	$sqlHistorialTmp = "SELECT * "
						."FROM ( "
							."SELECT a.Fproceso, a.tablaTemp, a.dateSys, "
								."(SELECT idcartera FROM ca_cliente_cartera WHERE idcliente_cartera=a.idcliente_cartera LIMIT 1) AS idcartera "
								."FROM ( "
								."SELECT Fproceso, tablaTemp, dateSys,idcliente_cartera FROM ca_historial "
								."GROUP BY Fproceso,dateSys "
								.") a "
							."WHERE a.Fproceso='".$fecha_proceso."' "
							."ORDER BY a.dateSys DESC "
							.") b "
							."WHERE b.idcartera=".$idCartera." LIMIT 1";

	$prHisTmp = $connection->prepare($sqlHistorialTmp);
	$prHisTmp->execute();
	$dataHisTmp = $prHisTmp->fetchAll(PDO::FETCH_ASSOC);
	$dataTabla = $dataHisTmp[0]['tablaTemp'];
	$dataArchivo = "No_existe_files.txt";
	//~ Vic F
...
//	$dataArchivo = $dataCartera[0]['archivo'];
	...
	if( $dataTabla == '' || $dataTabla == 'temporal eliminada' ) {
		...
		$sqlLV = " SELECT tmp.Fproceso, tmp.agencia, tmp.Gestor, tmp.territorio, tmp.oficina, tmp.subprod, tmp.nom_subprod, tmp.contrato, 
				tmp.codcent, tmp.tpersona, tmp.Nombre, tmp.divisa, CAST(tmp.saldohoy AS DECIMAL(12,2)) AS saldohoy, tmp.direccion, tmp.telefono, tmp.codpostal,
				tmp.codestado, tmp.codprovincia, tmp.coddistrito, tmp.fincumpli, tmp.diavenc, tmp.ubigeo, tmp.dpto, tmp.dist_prov, tmp.tramo_dia, tmp.tipodoc,
				tmp.nro_doc, tmp.tipofono1, tmp.prefino1, tmp.fono1, tmp.extension1, tmp.tipofono2, tmp.prefijo2, tmp.fono2, tmp.extension2, tmp.tipofono3,
				tmp.prefijo3, tmp.fono3, tmp.extension3, tmp.tipofono4, tmp.prefijo4, tmp.fono4, tmp.extension4, tmp.tipofono5, tmp.prefijo5, tmp.fono5,
				tmp.estension5, tmp.email, tmp.provision, tmp.fecha_de_generacion, tmp.fecha_de_ingreso, tmp.oficina2, tmp.producto, tmp.marca, tmp.tipo_telefono,
				tmp.prefijo_de_telefono, tmp.telefono1, tmp.extension, tmp.tipo_telefono2, tmp.prefijo_de_telefono2, tmp.telefono2, tmp.extension02, tmp.direccion1,
				tmp.departamento, tmp.provincia, tmp.distrito, tmp.direccion2, tmp.departamento2, tmp.provincia2, tmp.distrito2, tmp.idcartera, tmp.idcliente,
				tmp.idcliente_cartera, tmp.idcuenta, 
				".$str_implode_pago." 
				...
				DATE( cu.ul_fcpg ) AS 'UL_FCPG', REPLACE(cu.ul_observacion,'\n', '') AS 'UL_OBSERVACION',
