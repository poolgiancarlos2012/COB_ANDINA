<?php
    ...
    header("Content-Disposition: attachment; filename=reporte_diario_visitas_".$time.".txt");
    ...
    $sql = "    SELECT
                CONCAT(
                -- DATE_FORMAT(DATE(t1.fecha),'%d/%m/%Y'),'|',
                DATE_FORMAT(t1.fecha,'%d/%m/%Y'),' ',t1.hora_visita,'|',
                t1.contrato,'|',
                t1.rpta,
                ...
                ca_visita lla
                WHERE
                DATE(lla.fecha_visita) BETWEEN '$fecha_inicio' AND '$fecha_fin' AND
                lla.estado=1 AND
                lla.tipo='VIS'
                ORDER BY lla.idcliente_cartera DESC,lla.fecha_visita DESC
                ) t1
                WHERE
                CONCAT(
                DATE_FORMAT(t1.fecha,'%d/%m/%Y'),' ',t1.hora_visita,'|',
