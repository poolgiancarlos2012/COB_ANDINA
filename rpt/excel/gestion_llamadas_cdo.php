<?php
    ...
    $sql_llamada= "SELECT lla.idllamada,cu.dato9 as 'Territorio',car.nombre_cartera as 'CARTERA',CONCAT('=\"',clicar.codigo_cliente,'\"') AS 'CODIGO_CLIENTE',
                ...
                (SELECT prioridad FROM ca_final_servicio where idfinal=lla.idfinal and estado=1 AND idservicio=$Servicio ) AS 'Prioridad',
                lla.fecha_cp AS 'FECHA_CP',IF(cu.moneda='USD',$tipocambio*lla.monto_cp,IF(cu.moneda='VAC',$tipovac*lla.monto_cp,lla.monto_cp)) AS 'MONTO_CP',replace(replace(replace(Replace(Replace(Replace(lla.observacion,'|',''),char(10),''),char(13),''),char(9),''),char(8),''),char(34),'') AS 'OBSERVACION',
                ...
                motpag.nombre AS 'MOTIVO_NO_PAGO',lla.tipo AS 'MARCA'
                FROM ca_llamada lla
