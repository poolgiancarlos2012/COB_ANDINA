<?php
	...
	class PGSQL_PDODatosAdicionalesDetalleCuentaDAO {
		
		public function queryByDetalleCuenta ( dto_servicio $dtoServicio, dto_detalle_cuenta $dtoDetalleCuenta, dto_cartera $dtoCartera ) {
			$sql=" SELECT dato1,dato2,dato3,dato4,dato5,dato6,dato7,dato8,dato9,dato10,dato11,dato12,dato13,dato14,dato15,
				...
				dato48,dato49,dato50
 				FROM ca_cabeceras WHERE idservicio=? AND idtipo_datos_adicionales=3 AND idcartera = ? 
				UNION
				...
				IFNULL(dato48,'') AS 'dato48',IFNULL(dato49,'') AS 'dato49',IFNULL(dato50,'') AS 'dato50'
 				FROM ca_datos_adicionales_detalle_cuenta WHERE codigo_operacion =? AND idcartera = ? ";
				
			$servicio=$dtoServicio->getId();
			$cartera=$dtoCartera->getId();
			//$DetalleCuenta=$dtoDetalleCuenta->getId();
			$CodigoOperacion=$dtoDetalleCuenta->getCodigoOperacion();
			...
			//$pr->bindParam(3,$DetalleCuenta);
			/*****/
			$pr->bindParam(3,$CodigoOperacion);
			/*****/
