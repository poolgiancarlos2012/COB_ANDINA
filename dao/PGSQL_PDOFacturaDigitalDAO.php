...
class PGSQL_PDOFacturaDigitalDAO {
    ...
    public function getLinasFacturaDigital($idClienteCartera) {
        $factoryConnection = FactoryConnection::create('postgres_pdo');
        $connection = $factoryConnection->getConnection();
        $sql = 'SELECT cuenta.idcuenta,telefono FROM ca_cuenta as cuenta LEFT JOIN ca_factura_digital as fact ON  cuenta.idcuenta = fact.idcuenta WHERE cuenta.idcliente_cartera = ? AND (fact.estado = 1 OR ISNULL(fact.estado)) AND cuenta.estado = 1  AND (fact.is_send = 0 OR ISNULL(fact.is_send))';
        $stm = $connection->prepare($sql);
