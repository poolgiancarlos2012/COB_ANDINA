<?php
	...
	class PGSQL_PDOAyudaGestionUsuarioDAO {
		...
		public function queryListarUsuariosAsignados ( dto_cliente_cartera $dtoClienteCartera,$idusuario_servicio ) {
			...
			( SELECT CONCAT_WS(' ',usu.nombre,usu.paterno,usu.materno) FROM ca_usuario usu INNER JOIN ca_usuario_servicio ususer ON ususer.idusuario=usu.idusuario WHERE ususer.idusuario_servicio = idusuario_servicio_ayuda_gestion LIMIT 1 ) AS 'nombre' 
			FROM ca_ayuda_gestion_usuario WHERE idcartera IN (".$cartera.") AND estado = 1 and idusuario_servicio=".$idusuario_servicio;
			$cartera=$dtoClienteCartera->getIdCartera();
