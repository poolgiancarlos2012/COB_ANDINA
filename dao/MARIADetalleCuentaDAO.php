...
class MARIADetalleCuentaDAO {

    public function updateFecVenc ( $idcartera, $fec_venc, $usuar_modf ) {
        
        $sqlUpdate = " UPDATE ca_detalle_cuenta 
                    SET fecha_vencimiento = ? , usuario_modificacion = ? 
                    WHERE idcartera = ? ";

        $factoryConnection = FactoryConnection::create('mysql');
        $connection = $factoryConnection->getConnection();            

        $pr = $connection->prepare( $sqlUpdate );
        $pr->bindParam(1,$fec_venc,PDO::PARAM_STR);
        $pr->bindParam(2,$usuar_modf,PDO::PARAM_INT);
        $pr->bindParam(3,$idcartera,PDO::PARAM_STR);
        if( $pr->execute() ) {
            
            $sqlCar = " SELECT idcartera, detalle_cuenta FROM ca_cartera WHERE idcartera = ? ";

            $prCar = $connection->prepare($sqlCar);
            $prCar->bindParam(1,$idcartera,PDO::PARAM_INT);
            $prCar->execute();
            $dataCar = $prCar->fetchAll(PDO::FETCH_ASSOC);

            $det_cu = json_decode( $dataCar[0]['detalle_cuenta'], true );
            $count_det_cu = 0;

            for( $i=0;$i<count($det_cu);$i++ ) {
                
                if( $det_cu[$i]['campoT'] == 'fecha_vencimiento' ) {
                    $count_det_cu++;
                }
            ...
            if( $count_det_cu == 0 ) {
                array_push( $det_cu, array("campoT"=>"fecha_vencimiento","dato"=>"FEC_VEN","label"=>"FECHA_VENCIMIENTO") );
            }

            $sqlUptCar = " UPDATE ca_cartera 
                        SET detalle_cuenta = ?, usuario_modificacion = ? 
                        WHERE idcartera = ? ";

            $prUptCar = $connection->prepare($sqlUptCar);
            $prUptCar->bindParam(1,json_encode( str_replace("\\","",$det_cu) ),PDO::PARAM_STR);
            $prUptCar->bindParam(2,$usuar_modf,PDO::PARAM_INT);
            $prUptCar->bindParam(3,$idcartera,PDO::PARAM_INT);
            if( $prUptCar->execute() ) {
                return array("rst"=>true,"msg"=>"Fecha Agregada correctamente");
            }else{
                return array("rst"=>false,"msg"=>"Error al agregar fecha");
            }

        }else{
            return array("rst"=>false,"msg"=>"Error al actualizar detalle cuenta");
        }
    ...
    public function countClientesDisponiblesPorTramoEspecial(dto_detalle_cuenta $dtoDetalleCuenta) {
        ...
        $sql="  SELECT count(*) AS 'COUNT' FROM (
                ...
                )A WHERE A.cantidad BETWEEN ".$tramo." AND A.codigo_cliente IN (SELECT clicar.codigo_cliente FROM ca_cliente_cartera clicar where clicar.estado=1 and clicar.idcartera=$idcartera and clicar.idusuario_servicio=0)";
