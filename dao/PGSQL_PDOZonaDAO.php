<?php
	...
	class PGSQL_PDOZonaDAO {
		...
		public function insertDepartamentos ( $idservicio, $usuario_creacion, $idcartera ) {
			...
			FROM ca_direccion WHERE idcartera = ? AND TRIM(departamento)!='' AND ISNULL(departamento)=0 GROUP BY TRIM(departamento) ";
		...
		public function updateByService ( $idcartera, $data ) {
			...
			for( $i=0;$i<count($data);$i++ ) {
				...
				$sql = " UPDATE ca_zona SET zona = ? WHERE idzona = ? ";
				$pr = $connection->prepare($sql);
				...
				$sqlUpdateDireccion = " UPDATE ca_direccion SET zona = ? WHERE idcartera = ? AND TRIM(departamento) = ( SELECT departamento FROM ca_zona WHERE idzona = ? ) ";
